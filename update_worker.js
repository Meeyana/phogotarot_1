...
const kvSaveEndpoint = 
        if (urlParams.searchParams.get('action') === 'kv_save') {
          ...
          if (!env.SESSION) throw new Error("Chua c?u hình SESSION KV");
          ...
          for (const file of files) {
            ...
            if (match) {
              try {
                // very simple yaml parse for testing, ideally use library, but we just store what we have or parse line by line
                const lines = match[1].split('\\n');
                for (const line of lines) {
                  const colonIdx = line.indexOf(':');
                  if (colonIdx > -1) {
                    const k = line.substring(0, colonIdx).trim();
                    const v = line.substring(colonIdx + 1).trim().replace(/^['"](.*)['"]$/, '');
                    frontmatter[k] = v;
                  }
                }
    ...
    if (action === 'kv_list' || action === 'kv_get') {
        try {
            ...
            if (action === 'kv_list') {
                ...
                // dataStr is already JSON with rawMarkdown, content, frontmatter
                // We return it wrapped
